...
from bblocks.importers.config import Fields, logger, DataExtractionError
from bblocks.importers.data_validators import DataFrameValidator
from bblocks.importers.baci.data_manager import BaciDataManager
...
def _parse_data_links(soup: BeautifulSoup) -> dict[str, str]:
    ...
    try:
        logger.debug("Parsing BACI data links")
...
def extract_data_links() -> dict[str, str]:
    ...
    logger.info("Extracting BACI data links")
