...
from bblocks.importers.config import logger, Fields, DataExtractionError
...
def get_wb_databases() -> pd.DataFrame:
    ...
    sources = [i for i in wb.source.list()]
    df = (
        pd.json_normalize(sources)
...
def get_wb_entities(db: int | None = None, skip_aggs: bool = False) -> pd.DataFrame:
    ...
    economies = [i for i in wb.economy.list(db=db, labels=True, skipAggs=skip_aggs)]
    df = pd.json_normalize(economies, sep="_")
...
def get_wb_indicators(db: int | None = None) -> pd.DataFrame:
    ...
    series = [i for i in wb.series.list(db=db)]
    df = pd.json_normalize(series).rename(
...
def get_wb_indicator_metadata(
    ...
    if not metadata:
        raise DataExtractionError("No metadata found for indicator code(s).")
    ...
    if len(metadata) != len(indicator_code):
        raise DataExtractionError("Metadata not found for some indicator code(s).")
...
def _request_data(api_params: dict) -> pd.DataFrame:
    ...
    try:
        records = [i for i in wb.data.fetch(**api_params)]
