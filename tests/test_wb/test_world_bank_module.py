...
def world_bank(monkeypatch, tmp_path_factory):
    ...
    from bblocks.importers import utilities as utils

    if not hasattr(utils, "disk_memoize"):
        utils.disk_memoize = lambda *_, **__: lambda fn: fn

    sys.modules.pop("bblocks.importers.world_bank.world_bank", None)
    wb_module = importlib.import_module("bblocks.importers.world_bank.world_bank")
    yield wb_module
