...
from bblocks.importers.baci import baci
...
def test_extract_data_links_calls_helpers():
    with (
        mock.patch(
            "bblocks.importers.baci.baci._get_soup", return_value="soup"
        ) as mock_soup,
        mock.patch(
            "bblocks.importers.baci.baci._parse_data_links",
...
def test_validate_hs_version_cleans_and_validates():
    with mock.patch(
        "bblocks.importers.baci.baci.extract_data_links",
        return_value={"HS22": "link"},
    ):
...
def test_extract_data_success():
    ...
    with (
        ...
        mock.patch(
            "bblocks.importers.baci.baci.BaciDataManager", DummyManager
        ) as mock_manager,
        mock.patch(
            "bblocks.importers.baci.baci.DataFrameValidator",
...
def test_extract_data_raises_without_metadata():
    ...
    with (
        ...
        mock.patch("bblocks.importers.baci.baci.BaciDataManager", DummyManager),
        mock.patch("bblocks.importers.baci.baci.DataFrameValidator"),
    ):
...
def test_baci_get_data_includes_labels():
    ...
    with (
        mock.patch(
            "bblocks.importers.baci.baci._validate_hs_version", return_value="HS22"
        ),
