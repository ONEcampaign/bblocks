...
from bblocks.importers.imf import dsa
...
def test_get_dsa_returns_expected_dataframe(sample_raw_table, expected_clean_df):
    with (
        mock.patch("bblocks.importers.imf.dsa._download_pdf", return_value=b"pdf"),
        mock.patch(
            "bblocks.importers.imf.dsa._pdf_to_df", return_value=sample_raw_table
        ),
    ):
        result = dsa.get_dsa()
