...
from bblocks.importers.wfp import wfp
from bblocks.importers.wfp.wfp import (
    extract_countries,
    WFPInflation,
    WFPFoodSecurity,
    Fields,
)
from bblocks.importers.config import DataExtractionError, DataFormattingError
from bblocks.importers.utilities import (
    convert_dtypes,
)
...
def test_protocol():
    ...
    assert hasattr(importer_obj, "get_data"), (
...
class TestInflation:
    ...
    def test_extract_data_after_cache_clear(
        ...
        """
        # Clear cache
        with mock.patch("bblocks.importers.config.logger.info") as mock_logger:
    ...
    def test_load_data_country_not_available(self, wfp_inflation_pre_load):
        ...
        """
        # Mock logs
        with mock.patch("bblocks.importers.config.logger.warning") as mock_logger:
            # Call `load_data` method
    ...
    def test_load_data_successful(self, wfp_inflation_pre_load):
        ...
        with (
            mock.patch("bblocks.importers.config.logger.info") as mock_logger,
            mock.patch.object(
    ...
    def test_load_data_empty_data(self, wfp_inflation_pre_load):
        ...
        # Mock logs, `extract_data` and `format_data` methods
        with (
            mock.patch("bblocks.importers.config.logger.warning") as mock_warning,
            mock.patch.object(
                wfp_inflation_pre_load, "extract_data", return_value=mock_extracted_data
            ) as mock_extract,
            mock.patch.object(
    ...
    def test_get_data_no_valid_countries(self, wfp_inflation_post_load):
        ...
        with mock.patch(
            "bblocks.importers.utilities.convert_countries_to_unique_list",
            return_value=[],
        ):
            # Assertions
    ...
    def test_get_data_no_data_found(self, wfp_inflation_post_load):
        ...
        # Mock logger warning
        with mock.patch("bblocks.importers.config.logger.warning") as mock_warning:
            # Call method
            result = wfp_inflation_post_load.get_data(
                indicators="Headline inflation (YoY)", countries=["USA"]
            )
...
class TestFoodSecurity:
    ...
    def test_load_data_country_not_available(self, wfp_foodsecurity_pre_load):
        ...
        Mocks the logger but not the transport, so the available country in the
        pair is fetched from the live HungerMap API and the test fails whenever
