...
def test_get_data():
    ...
    #test dates
    df = pk_obj_1.get_data(start_date='2020_01_01', end_date = '2021-09-01')
    assert min(df.period) == '2020_01_01'
    assert max(df.period) == '2021-09-01'

    with pytest.raises(ValueError) as error:
        #check when dates are inverted
        pk_obj_1.get_data(start_date='2021-09-01', end_date = '2020_01_01')
        assert error.match('start date cannot be earlier than end date')

    #test no data returned
    with pytest.raises(ValueError) as error:
        pk_obj_1.get_data(start_date='2100-01-01')
