...
from bblocks.importers.undp import hdi
...
def test_read_hdi_data_success(mock_hdi_csv_content):
    ...
    """
    with mock.patch("bblocks.importers.undp.hdi._request_hdi_data") as mock_request:
        mock_response = mock.Mock()
        mock_response.content = mock_hdi_csv_content
...
def test_read_hdi_data_parser_error():
    ...
    with mock.patch(
        "bblocks.importers.undp.hdi._request_hdi_data", return_value=mock_response
    ):
...
def mock_read_hdi_data_func(mock_hdi_df):
    ...
    with mock.patch(
        "bblocks.importers.undp.hdi.read_hdi_data", return_value=mock_hdi_df
    ):
        yield
...
def mock_read_hdi_metadata_func(mock_metadata_df):
    ...
    with mock.patch(
        "bblocks.importers.undp.hdi.read_hdi_metadata",
...
def test_clear_cache(mock_read_hdi_data_func, mock_read_hdi_metadata_func):
    ...
    assert hdi_importer._data_df is None, (
        "_data_df should be None after clearing cache."
    )
    assert hdi_importer._metadata_df is None, (
